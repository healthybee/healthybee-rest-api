...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Healthybee Api Rest account.<br>
        ...
        &mdash; Healthybee Api Rest Team
      `
      return sendMail({ toEmail: email, subject: 'Healthybee Api Rest - Password Reset', content })
    })
